...
async function ingest(request: Request, env: Env): Promise<Response> {
  ...
  const next = isGenerated && (!prior || prior === 'new' || prior === 'skipped') ? 'generated' : prior ?? 'new';

  await env.DB.batch([
    env.DB.prepare(`
      INSERT INTO jobs (
        cipher,title,description,skills_json,matched_json,budget,link,publish_time,score,tier,
        proposal,hook_type,screening_json,status,notified_at,generated_at,created_at,updated_at
      ) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
      ON CONFLICT(cipher) DO UPDATE SET
        ...
        status=CASE WHEN jobs.status IN ('new','skipped') AND excluded.status='generated' THEN 'generated' ELSE jobs.status END,
    ...
    `).bind(
      ...
      proposal, hookType, jsonArray(screening), next, event === 'notified' ? now : null,
      isGenerated ? now : null, now, now,
    ),
...
async function listJobs(url: URL, env: Env): Promise<Response> {
  ...
  if (tab === 'applications') conditions.push("status != 'new'");
  else if (tab === 'inbox') conditions.push("status = 'new'");

  if (rawQuery) {
    const cipher = rawQuery.match(/~[A-Za-z0-9_-]{8,}/)?.[0];
    if (cipher) {
      conditions.push('cipher = ?');
      bindings.push(cipher);
    } else {
      const tokens = rawQuery.toLowerCase().split(/[^a-z0-9+#.~_-]+/).filter(v => v.length >= 2).slice(0, 8);
      for (const token of tokens) {
        conditions.push(`(lower(title) LIKE ? OR lower(description) LIKE ? OR lower(proposal) LIKE ? OR lower(link) LIKE ? OR lower(cipher) LIKE ? OR lower(tags_json) LIKE ?)`);
        const like = `%${token}%`;
  ...
  const result = await env.DB.prepare(`SELECT * FROM jobs ${where} ORDER BY updated_at DESC LIMIT ?`)
...
async function stats(env: Env): Promise<Response> {
  const [funnel, hooks, weekly, totals] = await Promise.all([
    env.DB.prepare(`SELECT
      SUM(CASE WHEN applied_confirmed=1 THEN 1 ELSE 0 END) applied,
      ...
      FROM jobs`).first(),
    env.DB.prepare(`SELECT hook_type,
      SUM(CASE WHEN applied_confirmed=1 THEN 1 ELSE 0 END) applied,
      SUM(CASE WHEN status IN ('replied','interview','won') THEN 1 ELSE 0 END) replied,
      SUM(CASE WHEN status IN ('interview','won') THEN 1 ELSE 0 END) interview,
      SUM(CASE WHEN status='won' THEN 1 ELSE 0 END) won
      FROM jobs WHERE hook_type != 'unclassified' GROUP BY hook_type ORDER BY applied DESC`).all(),
    env.DB.prepare(`SELECT substr(COALESCE(applied_at,updated_at),1,10) day,
      COUNT(*) applied,
      SUM(CASE WHEN status IN ('replied','interview','won') THEN 1 ELSE 0 END) replies
      FROM jobs WHERE applied_confirmed=1 AND datetime(COALESCE(applied_at,updated_at)) >= datetime('now','-30 days')
      GROUP BY day ORDER BY day`).all(),
    env.DB.prepare(`SELECT COUNT(*) total,
      SUM(CASE WHEN status='new' THEN 1 ELSE 0 END) inbox,
      SUM(CASE WHEN status='generated' THEN 1 ELSE 0 END) likely,
      SUM(CASE WHEN applied_confirmed=1 THEN 1 ELSE 0 END) applied
      FROM jobs`).first(),
  ]);
  return json({ funnel: funnel ?? {}, hooks: hooks.results ?? [], weekly: weekly.results ?? [], totals: totals ?? {} });
}
